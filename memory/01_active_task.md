...
- [x] PT-1 session logs can now be prefilled from CLI outputs for objective fields
...
## Context
- **Recent achievements**: `rust_rifle` and `field_pack` are now real event-driven equipment rewards, CLI players can see acquisition and replacement in Chinese, PT-1 now has an operator packet plus automatic PASS/FAIL summary output, PT-2 readouts now classify mismatch reasons instead of only reporting raw match rates, and PT-1 session logs can be prefilled from CLI outputs so humans only need to finish the subjective observation fields.
